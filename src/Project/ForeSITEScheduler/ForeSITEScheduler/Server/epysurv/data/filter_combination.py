...
def _date_range_left_closed(start, end, freq):
    """Create a left-closed date range compatible with old and new pandas."""
    try:
...
class FilterCombination:
    ...
    def expanding_windows(
        ...
        true_train = (
            pd.DataFrame(
                index=_date_range_left_closed(offset, split_years.middle, freq=FREQ)
            )
        ...
        true_test = (
            pd.DataFrame(
                index=_date_range_left_closed(
                    split_years.middle, split_years.end, freq=FREQ
                )
            )
            .join(
    ...
    def _expanding_frame(
        ...
        for date in _date_range_left_closed(offset, end, freq=FREQ):
            ts = (
                data
                # .copy()
                .query(
