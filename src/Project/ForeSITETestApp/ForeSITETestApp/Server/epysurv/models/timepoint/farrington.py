...
class FarringtonFlexible(STSBasedAlgorithm):
    ...
    def _check_enough_reference_data_available(self, detection_range, sts):
        dates = pd.to_datetime(np.asarray(sts.slots["epoch"]), unit="D", origin="unix")
        py_detection_range = (np.asarray(detection_range) - 1).astype(int)
        prediction_dates = dates[py_detection_range]
        required_reference_data = (
            prediction_dates - pd.DateOffset(years=self.years_back)
        ).floor("D")
